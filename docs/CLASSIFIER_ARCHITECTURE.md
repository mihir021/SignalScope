       ...
       [ Stream 1: Visual Semantic ]                                       [ Stream 2: Physical Forensic ]
          ...
          Visual Embeddings (512-d)                                           Forensic Embeddings (128-d)
                     ...
                     └─────────────────────────────────┬─────────────────────────────────┘
                                    ...
                                    Native Noise Autocorrelation (ρ)
                                       - Real CMOS Sensor: ρ < 0.08
                                       - Generative AI:    ρ > 0.15
                                                       │
                                                       ▼
...
| **Resolution** | Any arbitrary resolution ($500 \times 300$ up to $8000 \times 6000$) | No manual cropping required; full aspect ratio is preserved. |
...
When an image is evaluated via `predict_detailed()` or `POST /predict/detailed`, SignalScope produces **9 complementary forensic outputs**:
...
- **Why It Passed**: The visual stream saw the woven cot pattern and noted $62.9\%$ suspicion. However, Brain 2's physical sensor check confirmed genuine CMOS photon shot noise ($\rho = -0.0684$), preventing a false positive.
...
- **Why It Flagged**: High visual suspicion ($97.81\%$) corroborated by strong spatial noise autocorrelation ($\rho = +0.1674$), indicating generative upsampler deconvolution.
